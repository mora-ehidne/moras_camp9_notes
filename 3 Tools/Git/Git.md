...
The branch being pointed to by HEAD is the currently active (checked out) branch. When HEAD changes which branch it points to (switching between branches), the commit the new active branch is pointing to gets checked out (the working tree is set to its contents).
